use crate::daemon::client::DaemonHost;
use crate::host::{check_host, daemon_host_from};
...
#[derive(Parser)]
#[command(name = "reflex", version, about = "Run and manage Reflex Lua scripts")]
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
}
...
#[derive(Subcommand)]
pub enum Command {
    /// Open the Reflex graphical dashboard.
    Gui,
...
pub fn run(command: Command) -> Result<(), String> {
    match command {
        Command::Gui => {
            Err("the GUI command must be dispatched by the reflex application".to_string())
        }
...
pub fn run_script_detached(script: PathBuf) -> Result<(), String> {
...
fn list_scripts() -> Result<(), String> {
    print_scripts(&list_script_info()?);
    Ok(())
}

pub fn list_script_info() -> Result<Vec<ScriptInfo>, String> {
    let daemon = DaemonHost::connect_default().map_err(|err| err.to_string())?;
    daemon.list_scripts().map_err(|err| err.to_string())
}
...
fn stop_script(target: String) -> Result<(), String> {
    let script = stop_script_by_target(target)?;
    println!("stop requested for {} ({})", script.id, script.script_path);
    Ok(())
}

pub fn stop_script_by_target(target: String) -> Result<ScriptInfo, String> {
    let daemon = DaemonHost::connect_default().map_err(|err| err.to_string())?;
    daemon.stop_script(target).map_err(|err| err.to_string())
}
...
mod tests {
    ...
    #[test]
    fn parses_gui_subcommand() {
        let cli = Cli::try_parse_from(["reflex", "gui"]).unwrap();
